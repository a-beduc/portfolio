...
function initContactForm() {
    ...
    let localLang = localStorage.getItem("preferredLang") || "en";
    ...
    async function onSubmit(e) {
        localLang = localStorage.getItem("preferredLang") || 'en';
        e.preventDefault();
        msgBox.textContent = "";
        ...
        try {
            ...
            form.reset();
            flash(flashText[localLang].ok);
        } catch (err) {
            flash(flashText[localLang].ko);
        } finally {
            toggle(true);
        }
    }
    ...
    function verifyInput(inputKey, maxLength= 1000) {
        ...
        if (!value && inputKey !== 'company') {
            errorList.push(`${label} ${flashText[localLang].invalid}`);
        }
        if (value && value.length > maxLength) {
            errorList.push(`${label} ${flashText[localLang].tooLong}: ${maxLength}`);
        }
        return errorList;
    }
